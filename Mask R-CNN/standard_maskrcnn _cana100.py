...
VALSET_FILENAME = '/home/konsta/CanaTree100/annotations/fold_01/val_fold_01.json'
TESTSET_FILENAME = '/home/konsta/CanaTree100/annotations/fold_01/test_fold_01.json'
...
INITIAL_WEIGHTS = 'X-101_RGB_60k.pth'       # Path to a previous checkpoint to finetune or None
...
def init_maskrcnn(config_file, train_dicts, test_dicts, output_dir, fold_num=None, initial_weights=None): 
    ...
    cfg.DATASETS.TRAIN = ("cana_train",)
    cfg.DATASETS.TEST = ("cana_test",)
    ...
    MetadataCatalog.get(cfg.DATASETS.TRAIN[0]).set(thing_classes=["tree"])
    DatasetCatalog.register(cfg.DATASETS.TEST[0], lambda d=cfg.DATASETS.TEST[0]: test_dicts)
    MetadataCatalog.get(cfg.DATASETS.TEST[0]).set(thing_classes=["tree"])
...
if __name__ == "__main__":
    ...
    # Split train/test
    train_dicts = load_coco_json(DATASET_FILENAME, IMAGE_DIR)
    val_dicts = load_coco_json(VALSET_FILENAME, IMAGE_DIR)
    train_dicts = np.array(train_dicts + val_dicts)
